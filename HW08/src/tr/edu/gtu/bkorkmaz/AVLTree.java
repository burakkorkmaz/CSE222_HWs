...
/**
 * Created by Burak Kağan Korkmaz on 10.05.2017.
 */
...
public class AVLTree < E
    ...
    private AVLNode < E > delete(AVLNode < E > localRoot, E item) {
        ...
        if (item.compareTo(localRoot.data) == 0) {
            ...
            AVLTree.AVLNode <E> temp = findReplacementNode(localRoot);
            if(temp != null) {
                if (temp.balance > AVLNode.RIGHT_HEAVY) {
                    temp = rebalanceRightL(temp);
                }
                else if (temp.balance < AVLNode.LEFT_HEAVY) {
